...
console.log(getPINs('111111'));

function getPINs(observed) {
  const combinations = [];
  observed.split('').forEach(val => combinations.push(getNeighbors(val)));
  const results = [];
  createChildren('', combinations, 0, results);
  return results;
}

function createChildren(first, combinations, c, results) {
  if (first.length === combinations.length) results.push(first);
  else combinations[c].forEach(val => createChildren(first + val, combinations, c + 1, results));
}
